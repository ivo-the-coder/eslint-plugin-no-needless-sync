...
const flattenDependencySets = (directDependencySets) => {
  directDependencySets.forEach((dependencySet, index) => {
    for (let dependency of dependencySet) {
      directDependencySets[index] = union(
        dependencySet,
        directDependencySets[dependency]
      );
    }
  });
};
  ...
  nodes.forEach((node, index) => {
    if (isSynchronised(node)) {
      ...
      priorNodeIndices.forEach((priorIndex) => {
        if (
          isSynchronised(nodes[priorIndex]) &&
          !dependencySets[index].has(priorIndex)
        ) {
          results.add(index);
...
const findNeedlesslySynchronisedAwaitExpressions = (blockStatement) => {
  ...
  const independentAwaitIndices = findMutuallyIndependentAwaitIndices(
    dependencySets,
    blockStatement.body
  );
