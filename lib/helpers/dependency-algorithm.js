const { isSynchronised, findRelevantExpressions, findVariableIdentifierNames, findArgumentIdentifierNames } = require('./finders');
...
const findNeedlesslySynchronisedAwaitExpressions = (blockStatement) => {
  const relevantExpressions = findRelevantExpressions(blockStatement);
  const dependencySets = directDependencySets(relevantExpressions);
  flattenDependencySets(dependencySets);
  const independentAwaitIndices = findMutuallyIndependentAwaitIndices(dependencySets, relevantExpressions);

  return independentAwaitIndices.map(index => relevantExpressions[index]);
}
