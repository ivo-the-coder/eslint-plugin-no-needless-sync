...
describe('ArrayExpression', () => {
  describe('is', () => {
    test('return true if the predicate is true for any of the elements', () => {
      const node = { elements: [1, 2, 3] };
      const predicate = jest.fn(x => x % 2 == 0)
      expect(ArrayExpression.is(node, predicate)).toBe(true)
    })
    test('return false if the predicate is true for none of the elements', () => {
      const node = { elements: [1, 3, 5] };
      const predicate = jest.fn(x => x % 2 == 0)
      expect(ArrayExpression.is(node, predicate)).toBe(false)
    })
  })
